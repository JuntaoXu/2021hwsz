...
class ImageClassificationService(PTServingBaseService):
    ...
    def _preprocess(self, data):
        ...
        for k, v in data.items():
            for file_name, file_content in v.items():
                ...
                img1 = img.crop((0, 0, int(width*2/3), height))
                img2 = img.crop((int(width / 3), 0, width, height))
                preprocessed_data[k] = [torch.unsqueeze(F.to_tensor(img1),dim=0).to(self.device),
                                        torch.unsqueeze(F.to_tensor(img2),dim=0).to(self.device)]
        return preprocessed_data, width

    def _inference(self, data, width):
        img = data["input_img"]
        data = img
        # calculate the result of split images
        result1 = self.model(data[0])
        result2 = self.model(data[1])
        for idx in range(len(result1)):
            result1[idx]['boxes'] = result1[idx][
                'boxes'].cpu().detach().numpy().tolist()
            result1[idx]['labels'] = result1[idx][
                'labels'].cpu().detach().numpy().tolist()
            result1[idx]['scores'] = result1[idx][
                'scores'].cpu().detach().numpy().tolist()
        for idx in range(len(result2)):
            boxes = result2[idx][
                'boxes'].cpu().detach().numpy().tolist()
            boxes = [[b[0]+width/3, b[1], b[2]+width/3, b[3]] for b in boxes]
            result1[idx]['boxes'].extend(boxes)
            result1[idx]['labels'].extend(result2[idx][
                'labels'].cpu().detach().numpy().tolist())
            result1[idx]['scores'].extend(result2[idx][
                'scores'].cpu().detach().numpy().tolist())
    ...
    def _postprocess(self, data):
        ...
        for idx in range(len(result)):
            boxes = result[idx]['boxes']
            labels = result[idx]['labels']
            scores = result[idx]['scores']
            boxes, labels, scores = voter(boxes, labels, scores)
            '''
            threshold 0.8
            '''
            b, l, s = nms(boxes, labels, scores, threshold=0.85)
            result[idx]['boxes'] = b
            result[idx]['labels'] = l
            result[idx]['scores'] = s
            '''
            ...
            limit to 5
            '''
            result[idx]['boxes'] = b[:5]
            result[idx]['labels'] = l[:5]
            result[idx]['scores'] = s[:5]
    ...
    def inference(self, data):
        ...
        data,width = self._preprocess(data)
        ...
        data = self._inference(data, width)
